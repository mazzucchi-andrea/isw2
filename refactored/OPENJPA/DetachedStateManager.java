...
public class DetachedStateManager
    ...
    public Object attach(AttachManager manager, Object toAttach,
        ...
        StateManagerImpl sm = createStateManager(manager, toAttach, owner, ownerMeta, broker);
        PersistenceCapable pc = sm.getPersistenceCapable();
        manager.setAttachedCopy(toAttach, pc);
        manager.fireBeforeAttach(toAttach, meta);

        // Pre-load fields for efficiency
        FieldMetaData[] fields = sm.getMetaData().getFields();
        Object origVersion = preloadFields(sm, broker, fields);

        // Process fields
        processFields(manager, sm, pc, fields);

        // Perform version check and finalize
        finalizeAttach(sm, broker, origVersion, manager);

        return sm.getManagedInstance();
    }

    /**
     * Creates a state manager for the attached object.
     */
    private StateManagerImpl createStateManager(AttachManager manager, Object toAttach, 
            OpenJPAStateManager owner, ValueMetaData ownerMeta, BrokerImpl broker) {
        StateManagerImpl sm;
        if (_embedded) {
            if (_dirty.length() > 0)
    ...
    /**
     * Pre-loads fields for efficiency.
     */
    private Object preloadFields(StateManagerImpl sm, BrokerImpl broker, FieldMetaData[] fields) {
    ...
    /**
     * Processes all fields of the attached object.
     */
    private void processFields(AttachManager manager, StateManagerImpl sm, 
            PersistenceCapable pc, FieldMetaData[] fields) {
        ...
        for (int i = 0; i < fields.length; i++) {
            ...
            provideField(i);
            processField(manager, sm, pc, fields[i], i, loaded, set);
        }
        pc.pcReplaceStateManager(sm);
    }

    /**
     * Processes a single field based on its type.
     */
    private void processField(AttachManager manager, StateManagerImpl sm, 
            PersistenceCapable pc, FieldMetaData field, int i, BitSet loaded, int set) {
        switch (field.getDeclaredTypeCode()) {
            ...
            case JavaTypes.PC_UNTYPED:
                processPersistentField(manager, sm, pc, field, i, loaded, set);
                break;
            case JavaTypes.COLLECTION:
                processCollectionField(manager, sm, pc, field, i, loaded, set);
                break;
            case JavaTypes.MAP:
                processMapField(manager, sm, pc, field, i, loaded, set);
                break;
            default:
                if (_dirty.get(i))
    ...
    /**
     * Processes a persistent field.
     */
    private void processPersistentField(AttachManager manager, StateManagerImpl sm, 
            PersistenceCapable pc, FieldMetaData field, int i, BitSet loaded, int set) {
        if (field.getCascadeAttach() == ValueMetaData.CASCADE_NONE)
            objval = getReference(manager, objval, sm, field);
        else {
            PersistenceCapable toPC = null;
            if (objval != null && field.isEmbeddedPC())
                toPC = ImplHelper.toPersistenceCapable(objval,
                    manager.getBroker().getConfiguration());
            objval = manager.attach(objval, toPC, sm, field, false);
        }
        if (_dirty.get(i))
            sm.settingObjectField(pc, i, (!loaded.get(i)) ? null
                : sm.fetchObjectField(i), objval, set);
    ...
    /**
     * Processes a collection field.
     */
    private void processCollectionField(AttachManager manager, StateManagerImpl sm, 
            PersistenceCapable pc, FieldMetaData field, int i, BitSet loaded, int set) {
        Collection coll = (Collection) objval;
        objval = null;
        if (coll != null)
            coll = attachCollection(manager, coll, sm, field);
        if (_dirty.get(i))
            sm.settingObjectField(pc, i, (!loaded.get(i)) ? null
    ...
    /**
     * Processes a map field.
     */
    private void processMapField(AttachManager manager, StateManagerImpl sm, 
            PersistenceCapable pc, FieldMetaData field, int i, BitSet loaded, int set) {
        Map map = (Map) objval;
        objval = null;
        if (map != null)
            map = attachMap(manager, map, sm, field);
        if (_dirty.get(i))
            sm.settingObjectField(pc, i, (!loaded.get(i)) ? null
    ...
    /**
     * Performs version check and finalizes the attach process.
     */
    private void finalizeAttach(StateManagerImpl sm, BrokerImpl broker, 
            Object origVersion, AttachManager manager) {
