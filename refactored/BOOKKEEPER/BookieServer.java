...
public class BookieServer implements NIOServerFactory.PacketProcessor, BookkeeperInternalCallbacks.WriteCallback {
    ...
    public void processPacket(ByteBuffer packet, Cnxn src) {
        ...
        // Parse packet and extract basic information
        PacketInfo packetInfo = extractPacketInfo(packet, h);
        long ledgerId = packetInfo.ledgerId;
        long entryId = packetInfo.entryId;
        byte[] masterKey = packetInfo.masterKey;

        // Validate protocol version
        if (!isValidProtocolVersion(h, src, ledgerId, entryId)) {
            return;
        }
        ...
        case BookieProtocol.ADDENTRY:
            ...
            success = processAddEntry(packet, src, h, ledgerId, entryId, masterKey, flags, startTime);
            break;
        case BookieProtocol.READENTRY:
            statType = BKStats.STATS_READ;
            success = processReadEntry(packet, src, h, ledgerId, entryId, masterKey, flags);
            break;
        default:
            src.sendResponse(buildResponse(BookieProtocol.EBADREQ, h.getVersion(), h.getOpCode(), ledgerId, entryId));
        }

        updateStats(statType, success, startTime);
    }

    /**
     * Extract basic information from the packet.
     */
    private PacketInfo extractPacketInfo(ByteBuffer packet, PacketHeader h) {
        long ledgerId = -1;
        long entryId = BookieProtocol.INVALID_ENTRY_ID;
        byte[] masterKey = null;

        if (h.getOpCode() == BookieProtocol.ADDENTRY) {// first read master key
            masterKey = new byte[BookieProtocol.MASTER_KEY_LENGTH];
            packet.get(masterKey, 0, BookieProtocol.MASTER_KEY_LENGTH);
            ByteBuffer bb = packet.duplicate();
            ledgerId = bb.getLong();
            entryId = bb.getLong();
        } else if (h.getOpCode() == BookieProtocol.READENTRY) {
            ledgerId = packet.getLong();
            entryId = packet.getLong();
        }

        return new PacketInfo(ledgerId, entryId, masterKey);
    }

    /**
     * Validate the protocol version.
     */
    private boolean isValidProtocolVersion(PacketHeader h, Cnxn src, long ledgerId, long entryId) {
    ...
    /**
     * Process an add entry operation.
     */
    private boolean processAddEntry(ByteBuffer packet, Cnxn src, PacketHeader h, 
                                   long ledgerId, long entryId, byte[] masterKey, 
                                   short flags, long startTime) {
        try {
    ...
    /**
     * Process a read entry operation.
     */
    private boolean processReadEntry(ByteBuffer packet, Cnxn src, PacketHeader h, 
                                    long ledgerId, long entryId, byte[] masterKey, 
                                    short flags) {
        ...
        try {
            if ((flags & BookieProtocol.FLAG_DO_FENCING) == BookieProtocol.FLAG_DO_FENCING) {
                processFencing(packet, src, h, ledgerId, masterKey);
            }
    ...
    /**
     * Process fencing operation as part of read request.
     */
    private void processFencing(ByteBuffer packet, Cnxn src, PacketHeader h, 
                               long ledgerId, byte[] masterKey) throws BookieException, IOException {
     ...
     */
    private void updateStats(int statType, boolean success, long startTime) {
        if (isStatsEnabled) {
            if (success) {
    ...
    /**
     * Class to hold packet information.
     */
    private static class PacketInfo {
        ...
        PacketInfo(long ledgerId, long entryId, byte[] masterKey) {
